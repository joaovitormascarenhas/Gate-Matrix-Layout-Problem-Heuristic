...
int avaliacao(int lines, vector < set <int> > tracksOfGates, vector <int> NGatesOfTracks, vector <int> orderOfGates, int &cost){
...
bool aprimoramentoAdaptativo2(vector < vector <int> > M, int lines, vector <int> &orderOfGates, vector <int> activeGates, vector < set <int> > tracksOfGates, vector <int> NGatesOfTracks, int LAHEAD){
	...
	initialCost=originalCost=avaliacao(lines, tracksOfGates, NGatesOfTracks, newOrder, originalCost);
	...
	do{
		...
		for (i=0; i<lines; i++){//Abrindo mais tarde
			...
			for(itr=onesPosition.rbegin(); itr!=onesPosition.rend(); ++itr){
				...
				if (originalCost>=avaliacao(lines, tracksOfGates, NGatesOfTracks, currentOrder, newCost)){
...
void GemeosEntreSi(vector < set <int> > LAdjacency, int lines, vector <int> &verticesAtivos, vector < set <int> > &verticesDominados){
	...
	vector <int> :: iterator it;
	for(i=0; i<lines; i++){
		for(j=0; j<lines; j++){//for(j=i+1; j<lines; j++){
			if(i!=j && verticesAtivos[i]==0 && verticesAtivos[j]==0){//if(verticesAtivos[i]==0 && verticesAtivos[j]==0){
...
int main(){ 
	...
	avaliacao(lines, tracksOfGates, NGatesOfTracks, orderOfGates, cost);
